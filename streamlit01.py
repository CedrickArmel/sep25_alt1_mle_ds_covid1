...
""")
    ...
    """)
        ...
        if g1.exists():
            col1, col2, col3 = st.columns([1,2,1])  # centrer + réduire largeur
            with col2:
                st.image(str(g1), caption="Distribution par patlogies des classes", width=350)
        else:
            st.warning("⚠️ Impossible de trouver `grafico2.png` dans le dossier reports/.")
# -------------------------------------------------------------------
...
""")
    ...
    if mode_rb.startswith("Mode Binaire"):
        st.subheader("Mode Binaire (Sain vs Malade)")
        st.markdown("""Regroupement des classes. Dans ce mode, on regroupe les catégories ainsi :
...
""")
    ...
    st.markdown("""Le dataset est séparé en trois sous‑ensembles afin d’assurer un entraînement fiable et une évaluation correcte du modèle.
    ...
    if not transfo_images:
        ...
        with c1:
            if st.button("◀️", key="transfo_prev_arch", use_container_width=True):
                st.session_state["transfo_idx_arch"] = (st.session_state["transfo_idx_arch"] - 1) % n
        with c3:
        ...
        cimg.image(str(img_path), caption=f"{title}  ({i+1}/{n})", use_container_width=True)
...
# =========================
    ...
    st.markdown("""    Cette section présente les architectures évaluées et leurs propriétés principales.
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            with col_c:
                st.image(
                    img, 
                    caption="Aperçu de l'image importée", 
                    width=350
                )
