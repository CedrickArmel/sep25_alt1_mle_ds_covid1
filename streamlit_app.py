...
# -------- 3) Analyse de l’aire pulmonaire & asymétrie (figures `pulmon*.png`) --------
    st.header("🫁 Analyse de l’aire pulmonaire & asymétrie (masques)")

    st.markdown("""
    Cette partie du notebook explore si la **différence d’aire entre le poumon gauche et le poumon droit**
    est liée au statut de santé (*sain vs pathologique*).  
    On introduit un **indice d’asymétrie** sans dimension :

    \[
    \\text{Indice d’asymétrie} = \\frac{|A_g - A_d|}{A_g + A_d}
    \]

    - Valeur proche de **0** → **forte symétrie** (poumons équilibrés).  
    - Valeur **élevée** → **asymétrie marquée**, possiblement associée à certaines pathologies.  
    L’analyse s’appuie sur les masques binarisés, l’extraction de composantes connexes et la sélection des **deux plus grandes régions** (poumons).
    """)

    # Helpers: tri "naturel" pulmon, pulmon1, pulmon2...
    def natural_sort_key(p: Path):
        tokens = re.findall(r'\d+|\D+', p.stem.lower())
        return [int(t) if t.isdigit() else t for t in tokens]

    def list_pulmon_figs_carousel() -> list[Path]:
        # Busca pulmon*.png en reports/ y reports/figures/
        candidates = []
        for pat in [str(REPORTS_DIR / "pulmon*.png"), str(REPORTS_FIG_DIR / "pulmon*.png")]:
            candidates.extend([Path(p) for p in glob.glob(pat)])
        # dedupe manteniendo el primero encontrado
        unique = {str(p): p for p in candidates}
        files = list(unique.values())
        files.sort(key=natural_sort_key)
        return files

    pulmon_figs = list_pulmon_figs_carousel()

    if not pulmon_figs:
        st.info("ℹ️ Aucune figure `pulmon*.png` trouvée dans `reports/`.")
    else:
        # Estado del carrusel (independiente del de Haralick)
        key = "pulmon_carousel"
        if f"{key}_pos" not in st.session_state:
        ...
        pos = st.slider("Position (asymétrie / outliers / masques hors cadre)", 1, n, st.session_state[f"{key}_pos"], key=f"{key}_slider")
        st.session_state[f"{key}_pos"] = pos
        idx = pos - 1
        ...
        with st.expander("📝 Interprétation (résumé)"):
            st.markdown("""
            - **Indice d’asymétrie** : mesure la dissymétrie relative des aires pulmonaires (0 = symétrie parfaite).  
            - **Classe Normal** : asymétrie généralement **faible** (distribution plus resserrée).  
            - **Classes pathologiques** (COVID, Lung Opacity, Viral Pneumonia) : asymétries **plus dispersées**, indiquant des altérations locales.  
            - **Outliers (IQR)** : masques atypiques (ex. poumons **hors cadre**, artefacts) à **retirer** avant l’entraînement.  
            - Après retrait des masques hors cadre et outliers, les distributions d’asymétrie deviennent plus **cohérentes**.
            """)
